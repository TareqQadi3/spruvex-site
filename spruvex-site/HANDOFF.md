...
المستخدم زوّدني بمفتاح Resend حقيقي (`re_MSVHKuDm...`) ونطاق `spruvex.com`
مُتحقَّق منه فعليًا بحسابه — لكن **لم أستطع اختباره فعليًا من هذه الجلسة**:
بيئة العمل (sandbox) تحجب أي اتصال خارجي لـ`api.resend.com` على مستوى الشبكة
(403 policy denial من بروكسي الخروج نفسه) — **نفس فئة القيد** التي منعت
اختبار Render فعليًا طوال هذا المشروع. جرّبت ذلك بطريقتين: `curl` مباشر
لـ`api.resend.com`، وتشغيل `email.ts` الفعلي بنفس المفتاح — **كلاهما فشل
بنفس النمط بالضبط** (403 من نفس البروكسي، وليس رفضًا حقيقيًا من Resend)،
مما يؤكد أن القيد بيئي وليس متعلقًا بصحة المفتاح أو الكود.

**لأي جلسة تكمل من هنا**: لا تُعِد تجربة هذا داخل نفس نوع البيئة المعزولة —
اطلب من المستخدم تشغيل استدعاء curl بسيط لـ`https://api.resend.com/emails`
من جهازه هو (ليس من الـsandbox) للتأكد أن المفتاح/النطاق يعملان، أو اطلب منه
نشر التغييرات فعليًا على Render (الذي لا يملك هذا القيد) والتحقق من هناك
مباشرة.

- `RESEND_API_KEY` حقيقي متوفر الآن (زوّدني به المستخدم، احفظه محليًا فقط
  بـ`.env.local`، **لا تضعه أبدًا بأي ملف يُرفع لـgit**).
- `RESEND_FROM_EMAIL=SpruVex R <info@spruvex.com>` — نطاق مُتحقَّق منه فعليًا.
- `ADMIN_ALERT_EMAIL=tqstore276@gmail.com` — بريد المستخدم الحقيقي، صالح
  للاختبار المباشر.
...
## 14) قائمة تحقّق نهائية — تفعيل Resend بالكامل (بريد تلقائي حقيقي)

المستخدم زوّد الجلسة بمفتاح Resend حقيقي (`re_MSVHKuDm...`، نطاق `spruvex.com`
مُتحقَّق منه فعليًا بحسابه، عنوان `info@spruvex.com`). الكود جاهز ومُختبر
منطقيًا بالكامل (§6, §7.3) — **المتبقي فعليًا إعدادات فقط، لا كود جديد**:

1. **على خدمة spruvex-site بـRender**: اضبط (Environment، `sync: false`
   بالفعل بـ`render.yaml`):
   - `RESEND_API_KEY` = المفتاح الحقيقي.
   - `RESEND_FROM_EMAIL` = `SpruVex R <info@spruvex.com>`.
   - `ADMIN_ALERT_EMAIL` = بريد مالك المنصة الذي يريد استلام تنبيهات التسجيل.
   → يُفعّل بريدَي "الترحيب" و"تنبيه الإدارة" (§6، نقطتان 2 و3) فقط.

2. **على خدمة spruvex-r بـRender (منفصلة تمامًا، إعداد مستقل)**: اضبط
   `RESEND_API_KEY` بنفس القيمة أو بمفتاح مستقل — هذا **وحده** ما يُفعّل
   `ResendOtpSender` بدل `DevOtpSender` (§4.4, §6 نقطة 1) ليصل **رمز
   التحقق نفسه** بالبريد فعليًا. **بدون هذه الخطوة تحديدًا، لن يصل أي مستخدم
   حقيقي رمز التحقق أبدًا** حتى لو عمل بريدا الترحيب/التنبيه تمامًا — هذا أهم
   بند بهذه القائمة.

3. **شرط أساسي سابق لكل ما سبق**: يجب حل §7.1 أولًا (تطابق
   `SPRUVEX_SITE_API_KEY`) — بدونه لا يصل التسجيل أصلًا لمرحلة إرسال أي بريد.

4. **التحقق الفعلي بعد الضبط**: سجّل تجربة مجانية حقيقية من الموقع المنشور
   فعليًا، وتأكد بريديًا من وصول: (أ) رمز التحقق من spruvex-r، (ب) بريد
   الترحيب بعد التحقق، (ج) تنبيه الإدارة. **لم يتم هذا التحقق بعد** (القيد
   الشبكي بالجلسة الحالية منع تجربته من هنا — §7.3).
...
- **§7.1** — السبب الجذري (تطابق `SPRUVEX_SITE_API_KEY`) لم يُتحقَّق منه على
  Render الفعلي بعد (أهم بند، حرج).
- **§7.2** — تعديل كلمة المرور/نوع النشاط بمستودع spruvex-r مكتوب ومُختبر
  محليًا لكن **غير مدفوع** لفرعه هناك بعد.
- **§7.3 / §14** — تفعيل Resend يحتاج ضبط قيم فعلية على خدمتَي Render
  (المفتاح متوفر الآن) + تحقق فعلي بعد النشر (لم يتم بعد).
- **§7.4** — لا جلسة دخول تلقائية بعد التحقق من OTP؛ المستخدم يحتاج تسجيل
  دخول يدوي لاحقًا بلوحة spruvex-r (بالبريد وكلمة المرور).
- **§13** — لوحة تواصل/شراكات (CRM) بخمسة أقسام — موثَّقة كمواصفة فقط، تحتاج
  قرار معماري من المستخدم قبل أي كود.
